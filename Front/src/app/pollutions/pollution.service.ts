...
export class PollutionService {
  ...
  private applyFilters(list: Pollution[], filters: PollutionFilters) {
    ...
    if (filters.type) {
      list = list.filter((pollution) => pollution.type === filters.type);
    }
    if (filters.city) {
    ...
    if (filters.status) {
      list = list.filter((pollution) => pollution.status === filters.status);
    }
    return list;
  }
...
function loadFromStorage(): Pollution[] {
  ...
  try {
    ...
    if (!raw) {
      saveToStorage(FALLBACK_POLLUTIONS);
      return [...FALLBACK_POLLUTIONS];
    }
    ...
    console.error('Impossible de lire localStorage, utilisation des données par défaut.', error);
    saveToStorage(FALLBACK_POLLUTIONS);
    return [...FALLBACK_POLLUTIONS];
  }
}
...
function saveToStorage(pollutions: Pollution[]) {
  ...
  try {
    window.localStorage.setItem(STORAGE_KEY, JSON.stringify(pollutions));
  } catch (error) {
